...
export const registerUser = createAsyncThunk(
  'auth/register',
  async (userData: RegisterRequestDto & { [key: string]: any }, { rejectWithValue }) => {
    try {
      // Pass user data directly to authService without transformation
      // Keep all fields as-is: name, phone, aadharNumber (buyer), panNumber/businessName (vendor), etc.
...
export const verifyForgotPasswordOtp = createAsyncThunk(
  ...
  async (resetData: { email: string; otp: string; newPassword: string }, { rejectWithValue }) => {
    try {
      // Use resetPassword endpoint for forgot password verification
