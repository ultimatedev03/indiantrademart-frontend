  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (login.fulfilled.match(result)) {
      ...
      if (result.payload.user && !result.payload.requiresOTP) {
        const rawRole = result.payload.user.role || (Array.isArray(result.payload.user.roles) ? result.payload.user.roles[0] : null);
        const normalizedRole = normalizeRole(rawRole);
        console.log('✅ Buyer login success, role:', { rawRole, normalizedRole });
        
        if (['USER', 'BUYER'].includes(normalizedRole)) {
          console.log('User authenticated, redirecting to dashboard/buyer');
          setTimeout(() => {
            router.push('/dashboard/buyer');
          }, 100);
        } else if (normalizedRole === 'VENDOR') {
          console.log('Vendor role detected, redirecting to vendor dashboard');
          setTimeout(() => {
          ...
          console.warn('⚠️ Unknown role after login, showing error', { rawRole, normalizedRole });
          dispatch(clearError());
  ...
  const handleOtpVerification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (verifyOtp.fulfilled.match(result)) {
        ...
        const rawRole = result.payload.user?.role || (Array.isArray(result.payload.user?.roles) ? result.payload.user?.roles[0] : null);
        const normalizedRole = normalizeRole(rawRole);
        console.log('🎯 User authenticated via OTP, role:', { rawRole, normalizedRole });
        ...
        if (['USER', 'BUYER'].includes(normalizedRole)) {
          ...
          console.error('❌ User role mismatch:', { rawRole, normalizedRole });
          alert('This account is not registered as a user. Please use the correct login portal.');
        }
